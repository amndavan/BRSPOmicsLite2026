...
# memperbaiki maptools yang error belum terinstall
...
if (length(gset) > 1) idx <- grep("GPL570", names(gset)) else idx <- 1
gset <- gset[[idx]]
...
volcano_data$threshold[volcano_data$adj.P.Val < 0.05 & volcano_data$logFC > 0] <- "Up"
volcano_data$threshold[volcano_data$adj.P.Val < 0.05 & volcano_data$logFC < 0] <- "Down"

ggplot(volcano_data, aes(x=logFC, y=-log10(adj.P.Val), color=threshold)) +
...
# PART I. Heatmap dari DEG
library(pheatmap)

# Ambil DEG signifikan (FDR < 0.05)
tT2 <- topTable(fit2, adjust="fdr", sort.by="B", number=Inf)
tT2 <- tT2[tT2$adj.P.Val < 0.05, ]

# Ambil matriks ekspresi dari gset
ex <- exprs(gset)

# Subset hanya gen DEG
ex <- ex[rownames(ex) %in% rownames(tT2), ]

# Urutkan sampel berdasarkan grup
ord <- order(gs)
ex <- ex[, ord]

# Scaling per gene (z-score)
ex <- t(scale(t(ex)))

# Heatmap menggunakan pheatmap
library(pheatmap)
rownames(ex) <- tT2$Gene.Symbol[match(rownames(ex), rownames(tT2))]
ex <- ex[!is.na(rownames(ex)) & rownames(ex) != "", ]
pheatmap(ex,
         ...
         cluster_cols = TRUE,
         scale = "none",
         fontsize_row = 10,   # ← kecilkan (default = 10)
         fontsize_col = 8,
         main = "Heatmap DEG (FDR < 0.05)")

#PART J. Cleaning dan export
tT2 <- merge(tT2, gene.df,
             by.x = "Gene.Symbol",
             by.y = "SYMBOL",
             all.x = TRUE)
tT2 <- tT2[!is.na(tT2$ENTREZID), ]
tT2 <- tT2[!duplicated(tT2$ENTREZID), ]
write.csv(tT2, "results/DEG_clean_with_ENTREZ.csv", row.names = FALSE)

dir.create("data", showWarnings = FALSE)
saveRDS(gset, file = "data/GSE18090_raw.rds")
write.csv(ex, "data/GSE18090_expression_matrix.csv")
dir.create("scripts")
...
for (obj in ls()) {
  if (is.data.frame(get(obj))) {
    write.csv(get(obj),
              file = paste0("results/", obj, ".csv"),
              row.names = FALSE)
  }
}
dir.create("figures")

# Figures exported manually
# GO KEGG performed via Enrichr
